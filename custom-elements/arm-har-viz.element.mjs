...
export default class ArmHarViz extends HTMLElement {
	...
	dataPropChanged (datasets = []) {
		console.log("#dataPropChanged this", this);
		...
		if (isGraphableData) {
			this.shadowRoot.getElementById("downloadChart").classList.remove("nodisplay");
			this.chart.data = {};
			...
			const options = {
				...
				animation: {
					onComplete: this.chartAnimationComplete.bind(this),
				}
			};
			...
			this.shadowRoot.getElementById("downloadChart").classList.add("nodisplay");
	...
	chartAnimationComplete () {
		...
		downloadLink.download = this.data
			.map((dataset) => dataset.metadata)
			.join("--")
			.replace(/["',.|\\\/]/gi, "");
		downloadLink.href = this.chart.toBase64Image();
	}
	...
	updatedAttrChanged (newVal) {
		if (newVal !== "true") return;
		else this.dataPropChanged.call(this, this.data);
	}
	...
	static get template () {
		...
		<a id="downloadChart" class="nodisplay" href="" download>Download</a>
